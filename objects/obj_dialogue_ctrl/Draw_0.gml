...
if IsChatterbox(chatterbox) and text != undefined
{
	...
	draw_set_halign(_me ? fa_left : fa_right);   // if the text is me it is left aligned otherwise it is right aligned
	...
	if ChatterboxGetOptionCount(chatterbox)
	{
		...
		for (var i = 0; i < ChatterboxGetOptionCount(chatterbox); i++;)
		{
			...
			{
				_xx = room_width / 2;
				_yy = (room_height / 6) * (i + 2);   //because in this instance there are only ever three options we can split room into 6 and then draw options at precission 2,3, and 4 to vertically center 
